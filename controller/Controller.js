...
class Controller {
  ...
  async checkAccess({ controller, action }, callback, ...args) {
    ...
    forbidden.error(
      `${userId} access denied. (permission: ${namespace}.${permissionName}. controller: ${controller}. action: ${action})`
    )
  ...
  generateOperationPermission({ ctx, controller: controllerName, action: actionName }) {
    ...
    return {
      namespace,
      permissionName: `${controller.substring(0, 1).toLowerCase()}${controller.substring(1)}.${action}`,
    }
  }
