...
function SM4() {
  ...
  this.getKey = function (key) {
    const keyBytes = stringToByte(key);
    if (keyBytes.length < 16) {
      for (let i = 0;i < 16 - keyBytes.length;i++) {
        keyBytes.push(0);
      }
    }
    return keyBytes;
...
function SM4Util() {
  ...
  this.encryptData_ECB = function (plainText) {
    try {
      ...
      const keyBytes = sm4.getKey(this.secretKey);
  ...
  this.decryptData_ECB = function (cipherText) {
    ...
    try {
      ...
      const decrypted = sm4.sm4_crypt_ecb(ctx, base64js.toByteArray(cipherText));
